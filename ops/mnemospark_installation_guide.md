...
After this, `/mnemospark_wallet` and `/mnemospark_cloud` are available in OpenClaw (they appear in the slash command list when you type `/`). The plugin does **not** write to `~/.openclaw/extensions/` itself; only `openclaw plugins install` does that.

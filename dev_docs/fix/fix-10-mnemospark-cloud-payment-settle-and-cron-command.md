...
3. **Optional (recommended):** Include **informational** flags in the emitted string for **operator context** (must match what the parser accepts):
...
- `mnemospark` — `src/proxy.ts` (`/mnemospark/payment/settle`) — raw: `https://raw.githubusercontent.com/pawlsclick/mnemospark/refs/heads/main/src/proxy.ts`

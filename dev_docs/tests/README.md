...
- **test-mnemospark_cloud-with-clawrouter.sh**: Runs a small subset of `/mnemospark_cloud` commands with the OpenClaw/ClawRouter gateway running, ensuring that mnemospark continues to use its own directories (`~/.openclaw/mnemospark/...`) and does not interfere with BlockRun wallet state.
...
9. `./test-mnemospark_cloud-with-clawrouter.sh`  
   Runs a small set of `/mnemospark_cloud` commands with ClawRouter present to ensure command coexistence and path isolation.
...
- `test-mnemospark_cloud-with-clawrouter.sh` expects `MNEMOSPARK_BACKEND_API_BASE_URL` to point at a reachable backend (real or stub). If no backend is running, the script may log a warning for `price-storage` failures but still passes if the primary coexistence checks succeed.
